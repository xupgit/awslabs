...
int main(int argc, char** argv)
{
    ...
    int h_B_input[MAX_LENGTH];                    // host memory for input vector
    int h_B_output[MAX_LENGTH];                   // host memory for output vector
    cl_mem d_B;                         // device memory used for a vector

    int h_Res_input[MAX_LENGTH];                    // host memory for input vector
    int h_Res_output[MAX_LENGTH];                   // host memory for output vector
   ...
   // Get Accelerator compute device
    ...
    d_A = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX,  sizeof(int) * number_of_words, &d_bank_ext[0], NULL);



    d_B = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX,  sizeof(int) * number_of_words, &d_bank_ext[0], NULL);



    d_Res = clCreateBuffer(context,  CL_MEM_READ_WRITE | CL_MEM_EXT_PTR_XILINX,  sizeof(int) * number_of_words, &d_bank_ext[0], NULL);
    ...
    err = clEnqueueWriteBuffer(commands, d_B, CL_TRUE, 0, sizeof(int) * number_of_words, h_B_input, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(commands, d_Res, CL_TRUE, 0, sizeof(int) * number_of_words, h_Res_input, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to source array h_Res_input!\n");
        printf("Test failed\n");
        return EXIT_FAILURE;
    }
    ...
    err |= clEnqueueReadBuffer( commands, d_Res, CL_TRUE, 0, sizeof(int) * number_of_words, h_Res_output, 0, NULL, &readevent );
    ...
    for (uint i = 0; i < number_of_words; i++) {
        if ((h_A_input[i] + 1) != h_A_output[i]) {
            printf("ERROR in KVAdd - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_A_input[i], h_A_input[i], h_A_output[i], h_A_output[i]);
            check_status = 1;
        }
    ...
    for (uint i = 0; i < number_of_words; i++) {
        if ((h_B_input[i] + 1) != h_B_output[i]) {
            printf("ERROR in KVAdd - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_B_input[i], h_B_input[i], h_B_output[i], h_B_output[i]);
            check_status = 1;
        }
    ...
    for (uint i = 0; i < number_of_words; i++) {
        if ((h_Res_input[i] + 1) != h_Res_output[i]) {
            printf("ERROR in KVAdd - array index %d (host addr 0x%03x) - input=%d (0x%x), output=%d (0x%x)\n", i, i*4, h_Res_input[i], h_Res_input[i], h_Res_output[i], h_Res_output[i]);
            check_status = 1;
        }
